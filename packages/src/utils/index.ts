...
export function validateRule(fieldRules: RuleProps[], fieldData: any): TValidateRule {
...
export function isHasInputTarget(evt: Event) {
  if (evt?.target) {
    if (typeof evt.target === 'object' && ('value' in evt.target || 'checked' in evt.target)) return true
    evt.stopPropagation?.()
  }
  return false
}
